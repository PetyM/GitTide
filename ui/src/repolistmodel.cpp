...
QModelIndex RepoListModel::index(int row, int column, const QModelIndex& parent) const {
    ...
    if (pr < 0 || pr >= static_cast<int>(rows_.size())) return {};
    if (row < 0 || row >= static_cast<int>(rows_[pr].children.size())) return {};
...
int RepoListModel::rowCount(const QModelIndex& parent) const {
    if (!parent.isValid()) return static_cast<int>(rows_.size());
...
int RepoListModel::columnCount(const QModelIndex& parent) const {
    if (parent.isValid() && parent.internalPointer() != nullptr) return 0;
    return 1;
}
...
QVariant RepoListModel::data(const QModelIndex& index, int role) const {
    ...
    if (ptr == 0) {
        ...
        if (pr < 0 || pr >= static_cast<int>(rows_.size())) return {};
        if (cr < 0 || cr >= static_cast<int>(rows_[pr].children.size())) return {};
        const auto& sub = rows_[pr].children[cr];
        switch (role) {
            case Qt::DisplayRole: return sub.displayName;
            case PathRole:        return sub.path;
            case MissingRole:     return sub.missing;
            default:              return {};
        }
    }
}
