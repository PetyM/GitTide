...
// redirects declared children into its internal contentItem (via `default
// property alias contentData: contentItem.data`), so `parent` here is that
// contentItem, not the Flickable — `parent.parent` is the actual Flickable.
WheelHandler {
    id: scroller
