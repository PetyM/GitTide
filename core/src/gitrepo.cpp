...
bool is_whole_file(const gittide::StageSelection& sel)
{
...
Expected<void> GitRepo::apply_partial(const StageSelection& sel, bool stage)
{
    ...
    std::string patch = build_patch(to_git_path(sel.path), fileDiff->hunks[hi], sel, /*reverse=*/!stage);
...
Expected<void> GitRepo::discard(const StageSelection& sel)
{
    ...
    std::string patch = build_patch(p, fileDiff->hunks[hi], sel, /*reverse=*/true);
...
Expected<std::string> GitRepo::commit(const CommitRequest& req)
{
    ...
    if (git_reference_name_to_id(&parent_oid, repo_, "HEAD") == 0 && git_commit_lookup(&parent, repo_, &parent_oid) == 0)
    {
        parents[0]   = parent;
        parent_count = 1;
    }
    ...
    git_oid commit_oid;
    rc = git_commit_create(&commit_oid, repo_, "HEAD", sig, sig, nullptr, req.message.c_str(), tree, parent_count, parents);
    if (parent)
...
Expected<std::vector<CommitNode>> GitRepo::log(unsigned limit) const
{
    ...
    while ((limit == 0 || count < limit) && git_revwalk_next(&oid, walk) == 0)
    {
        ...
        if (git_commit_lookup(&c, repo_, &oid) < 0)
            continue;
...
Expected<std::vector<std::filesystem::path>> GitRepo::submodules() const
{
    ...
    auto cb = [](git_submodule* sm, const char* /*name*/, void* pl) -> int
    {
        ...
        if (rel)
            p->out->push_back(*p->wd / from_git_path(rel));
        return 0;
    };
