...
TEST_CASE("build_patch emits no-newline marker for an EOF line", "[patch]") {
    DiffHunk h;
    h.oldStart = 1; h.oldLines = 1; h.newStart = 1; h.newLines = 1;
    // Old 'x' (no trailing newline) -> new 'y' (no trailing newline).
    DiffLine removed{DiffLineOrigin::Removed, 1, -1, "x"}; removed.noNewline = true;
    DiffLine added{DiffLineOrigin::Added, -1, 1, "y"};     added.noNewline = true;
